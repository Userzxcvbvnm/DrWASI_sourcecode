...
FILE_NAME = "fd_prestat_dir_name"


def parmu_fd_prestat_dir_name(temfile=f"../../../../../executedir/testcasepool/seeds/{FILE_NAME}_00000.c"):
    index = 1 
    code = readtem(temfile)

    pattern = r'if \(access_mode & O_RDONLY\)'
    newsens = f'if (access_mode == O_RDONLY)'
    res, modified_code = replace(code, pattern, newsens)
    if res == -1:
        print(modified_code)
        return  
    
    pattern = r'if \(access_mode & O_WRONLY\)'
    newsens = f'if (access_mode == O_WRONLY)'
    res, modified_code = replace(modified_code, pattern, newsens)
    if res == -1:
        print(modified_code)
        return 
    
    pattern = r'if \(access_mode & O_RDWR\)'
    newsens = f'if (access_mode == O_RDWR)'
    res, modified_code = replace(modified_code, pattern, newsens)
    if res == -1:
        print(modified_code)
        return 
