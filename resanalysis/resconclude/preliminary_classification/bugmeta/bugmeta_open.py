...
class BugMetaOpen(BugMeta):
    def __init__(self, runtime, bugmsg, problemdir, file2perstr, file2openstyle, item="OPEN"):
        self.item = item
    ...
    def get(self):
        ...
        mes = f"{mes}[Changed permission file]:{self.file2per}\n"
        mes = f"{mes}[Open file]:{self.file2openstyle}\n"
    ...
    def __eq__(self, other):
        if not isinstance(other, BugMetaOpen):
    ...
    def __hash__(self):
        return hash((self.runtime, self.item, self.bugmsg, self.file2per, self.file2openstyle))
