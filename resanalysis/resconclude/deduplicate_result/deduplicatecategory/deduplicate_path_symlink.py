...
def deduplicate_path_symlink(filename, version="None"):
    print(f"Dedusplicating PATH_SYMLINK_result ...")
    outputfile = f"{DIR}/PATH_SYMLINK_final.txt"
    if version == "new":
        outputfile = f"{DIR}/new_version/PATH_SYMLINK_final.txt"
    elif version == "old":
        outputfile = f"{DIR}/old_version/PATH_SYMLINK_final.txt"  
    ...
    for record in records:
        ...
        msg = BugMetaPATH_SYMLINK(runtime=runtime, bugmsg=bugmsg, problemdir=problemdir)
        add_msg(msg_set, msg)
