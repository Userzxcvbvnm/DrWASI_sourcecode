...
def deduplicate_fd_prestat_dir_name(filename, version="None"):
    print(f"Deduplicating FD_PRESTAT_DIR_NAME_result ...")
    outputfile = f"{DIR}/FD_PRESTAT_DIR_NAME_final.txt"
    if version == "new":
        outputfile = f"{DIR}/new_version/FD_PRESTAT_DIR_NAME_final.txt"
    elif version == "old":
        outputfile = f"{DIR}/old_version/FD_PRESTAT_DIR_NAME_final.txt"    
    ...
    for record in records:
        ...
        match = re.search(pattern, record)
        if match:
            modemsg = match.group(1)
            
        msg = BugMetaFD_PRESTAT_DIR_NAME(runtime=runtime, bugmsg=bugmsg, problemdir=problemdir, 
